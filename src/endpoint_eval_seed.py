...
Pre-registered acceptance bars (PROTOCOL.md, fixed before any run):
  flank_ratio <= 0.85 (stretch 0.75), center_ratio >= 1.15, H >= 0.95,
  functional-band vitality (all |offset| <= 10 deg channels above 0.01 —
  amendment A3; exact-zero dead-ring counting over-counts under any
  subtractive term because far-ring baselines are already ~1e-6..1e-26).
Run: PYTHONHASHSEED=0 python3 -B endpoint_eval_seed.py <final.pt> <out.json>
"""
