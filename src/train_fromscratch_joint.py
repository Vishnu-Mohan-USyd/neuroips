...
FROM-SCRATCH JOINT-PROBE LINEAGE (this copy)
--------------------------------------------
This file is the executed run file of the from-scratch joint-training probe
(study record: ``docs/study_record/fromscratch_joint_20260825/``). It is
byte-derived from the transient s=0.04 ladder harness state — this repo's
``src/train_sweep.py`` lineage with ``pred_inhib_strength`` 0.05 -> 0.04,
sha256 7eb46f6c2a3b22885574b3961ce97ba9a1224259dc6654075cc8421b0e25d821, the
exact state that trained the s=0.04 family donors — and differs from it by
EXACTLY two hunks (full diff in the study record's RUN_LOG.md):

  (a) the pretrain stage is skipped: ``main()`` replaces the ``run_pretrain``
      call with the ``fromscratch_init`` block below (same lineage as
      run_pretrain's opening — seed_everything -> build -> reference_values —
      so the init state and references are bitwise those of the two-stage
      run; verified against the donor checkpoint's stored references);
  (d) numbered weight snapshots every 500 steps in ``run_alpha`` for
      trajectory analysis.

Two protocol deltas need no source change: (b) the parameter policy from step
0 is the FULL arm set because ``run_alpha`` already trains
``set_axis_parameter_policy`` = cell (gru) + W_fb + circ_raw with local_comp
frozen — the pretrain policy simply never executes; (c) the 11000-step joint
budget (matched to 3000 pretrain + 8000 arm) is the CLI flag
``--axis-steps 11000``. Executed as: ``PYTHONHASHSEED=0 python3 -B
train_fromscratch_joint.py --out runs/joint --seed 8 --alphas <0.5|0.0>
--axis-steps 11000 --recurrent-cell rnn_tanh --device cuda:0``. The unused
``run_pretrain`` function is retained verbatim (minimal-diff discipline).
This docstring section is the only difference between this copy and the
executed file (comment-only, per the repo's packaging convention).
"""
