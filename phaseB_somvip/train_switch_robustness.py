...
SIGMA=1.0; LAM_READ=1.0; LAM_SAVE=0.2
...
def run_seed(seed):
    ...
    net=SimpleNet(use_circuit=True,context=True).to(device); read=nn.Linear(N,N).to(device)
    ...
    for s in range(1,4001):
        ...
        rd=F.cross_entropy(read(r_all+SIGMA*torch.randn_like(r_all)).reshape(-1,N),chan(theta).reshape(-1),reduction='none').reshape(128,12).mean(1)
    ...
    gctx=F.softplus(net.g_ctx_raw).item(); sw=out[1.0][0]>out[1.0][1] and out[-1.0][0]<out[-1.0][1]
    print(f"  seed{seed}: g_ctx={gctx:.3f} SWITCH={'YES' if sw else 'NO'}",flush=True)
    return sw
print("=== NOISY-READOUT ROBUSTNESS: seeds 1,2 ===",flush=True)
res=[run_seed(s) for s in (1,2)]
print(f"ROBUST seeds[1,2] switches={res}",flush=True)
print("ROBDONE",flush=True)
