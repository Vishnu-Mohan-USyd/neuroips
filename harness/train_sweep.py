...
def set_axis_parameter_policy(
    ...
    # Candidate 6 (addendum 4 section 4 edit 4): som_out_raw is DELETED —
    # the broad SOM route's gains are non-trainable buffers (anatomy).
...
def mechanism_statistics(net: tuned.SimpleTunedNet) -> dict[str, float]:
    ...
    tuned.CIRC_INDEX order), the FIXED broad-blanket buffers (Candidate 6:
    mean_m = m_fixed scalar, cv_m = 0 structurally), the broad-route ceiling
    product w_sf_fixed*m_fixed = C_FIELD, and the local competition strength.
    ...
    mean_m = float(net.m_fixed.item())
    cv_m = 0.0
    return {
        **named,
        "mean_m": mean_m,
        "cv_m": cv_m,
        # Candidate 6 (addendum 4): the broad route is anatomy; its strength
        # is the fixed ceiling constant w_sf_fixed * m_fixed = C_FIELD.
        "som_route_strength_times_mean_m": (
            float(net.w_sf_fixed.item()) * mean_m
        ),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(|v_{t-1}| >= 2); 0.0 reproduces the Phase-2 stream bitwise."
        ),
    )
