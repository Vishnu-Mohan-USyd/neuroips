...
- [`docs/paradigm_sign_mechanism.md`](docs/paradigm_sign_mechanism.md) — Phase 4-7 paradigm-sign two-mechanism account (Mech 1 V2-feedback channel-resolved gain modulation; Mech 2 W_rec-amplified non-V2 stim-statistics bias) + Phase 7 decoder-robustness caveat.
...
**Mechanistic account (Phase 4-7, 2026-05-04 → 2026-05-05).** R1+R2's dampening paradigms separate into two independent mechanisms once V2 feedback is ablated (`feedback_scale=0`). **Mech 1** (V2-feedback channel-resolved gain modulation, pi-modulated polarity at `src/model/network.py:307-318`) drives M3R native + modified, VCD-test3 native + modified, and the HMM C1 paired-fork V2-predictable strata; ablating V2 flips Δ_decC sign or drives it to ~0. **Mech 2** (non-V2 stim-statistics bias amplified ~25× by the L2/3 recurrent kernel `W_rec` at `src/model/populations.py:206-238` + `:274`) drives HMS / HMS-T native + modified; V2 ablation INCREASES |Δ| there (V2 was opposing the underlying dampening). Channel-level findings (Δr_stimch flip with pi-matching, |ΔL23_stimch| reduction by ~57% under W_rec ablation) are decoder-robust under both Dec C and Dec A. Phase 7 caveat: the per-paradigm Mech 1 vs Mech 2 verdict is decoder-dependent on 3/8 paradigms (M3R native, HMS-T native, VCD-test3 modified disagree Dec C vs Dec A); pi-Δ flip and W_rec-ablation Δ sign-flip on HMS / HMS-T are Dec-C-specific (Dec A saturates near +0.40 on V2-predictable subsets). Full account: `docs/paradigm_sign_mechanism.md`.
