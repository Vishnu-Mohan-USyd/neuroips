...
The paradigm choice, not the decoder choice, drives the sign. The 2026-04-24 rerun extends the 17-row matrix to 7 decoder columns (A, A′, B, C, D-raw, D-shape, E) — the hybrid finding holds on all 13 R1+R2 rows. Two legacy rows (a1 and b1 HMM C1) carry a small nominal Dec A vs Dec E sign disagreement (Δ_A ≈ −0.03 → Δ_E ≈ +0.03), but the Δ on these rows is unreliable: Dec E is under-trained Adam on dampened frozen L2/3, where Adam needs ≥ 20 000 steps (not 5 000) to converge — see Task #6 retraction in `docs/research_log.md` 2026-04-25 entry and `docs/R1R2_full_report.md` § 9.6. Treat sign-agreement claims that depend on 5k-retrain Δ on a1 / b1 HMM C1 as artefactual. Details: `RESULTS.md` § 11–§ 14, `docs/project_summary.md` § 15–§ 18, `ARCHITECTURE.md` § "Decoders", `results/cross_decoder_comprehensive_with_all_decoders.{json,md}`.
...
| **E** | Dec-A-spec (`Linear(36, 36)+bias`, Adam lr=1e-3, 5000 steps, seed 42) retrained per-ckpt on the **natural HMM stream with HMM's own stochastic task_state** (Markov p_switch=0.2 or Bernoulli-per-batch per each yaml); post-Stage-2. | 0.5467 | Close to Dec A on R1+R2 (+0.5 pp; `frac_same_pred(A′, E) = 0.9722`). On a1 / b1 caps at top-1 ~0.35 — not a representational dissociation but Adam-@-5k optimisation-insufficiency on dampened frozen L2/3 (20k Dec A′ retrain reaches 0.671 / 0.663, see Task #6 / `docs/research_log.md` 2026-04-25 entry). The 2 sign flags on a1 / b1 HMM C1 are artefactual under-trained-Adam Δ. |
