#!/usr/bin/env python3
"""Prediction-only learned H_pred->V1_SOM feedback assay.

This trains ctx->pred and a checkpointed H_pred->V1_SOM route from future-state
templates only, freezes both, then evaluates preregistered Richter rows. It does
not train or select on Q/activity/decoder/dampening metrics, and it disables all
explicit predicted-current and divisive suppressors.
"""
...
DEFAULT_CHECKPOINT_DIR = Path(
    ...
    "stage1_v1_state_learned_som_feedback_20260425"
)
BASELINE = {
...
class EvalRow:
    ...
    def row(self) -> Row:
        return Row(
            condition="frozen_learned_hpred_v1som_feedback",
            name=self.name,
            feedback_g_total=self.feedback_g_total,
            feedback_r=self.feedback_r,
            feedback_som_center_weight=0.10,
            note=(
                "Frozen prediction-only learned H_pred->V1_SOM feedback route; "
                "explicit predicted suppression and divisive gates disabled."
            ),
...
def _criteria(metrics: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "beats_original_natural_baseline_all_three": (
            q_ue > BASELINE["q_u_minus_e"]
            and activity_ue > BASELINE["activity_u_minus_e"]
            and decoder_ue > BASELINE["decoder_u_minus_e_keep_p_0.02"]
        ),
    }
...
def _row_markdown(row: Mapping[str, Any]) -> str:
    ...
    return (
        ...
        f"{row['criteria']['beats_original_natural_baseline_all_three']} | "
...
def _write_markdown(summary: Mapping[str, Any], path: Path) -> None:
    ...
    lines.extend([
        ...
        "## Baseline Comparator",
        "",
        f"Original natural baseline Q U-E: `{BASELINE['q_u_minus_e']}`",
        f"Original natural baseline activity U-E: `{BASELINE['activity_u_minus_e']}`",
        f"Original natural baseline decoder U-E @ keep_p=.02: `{BASELINE['decoder_u_minus_e_keep_p_0.02']}`",
    ])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    out_dir = args.stage3_dir / "learned_som_feedback_20260425"
    out_dir.mkdir(parents=True, exist_ok=True)

    checkpoint = args.checkpoint_dir / (
        f"stage1_ctx_pred_v1_template_learned_som_seed{args.seed}_n{args.n_trials}.json"
    )
    heldout_eval = args.checkpoint_dir / (
        f"stage1_ctx_pred_v1_template_learned_som_seed{args.seed}_n{args.n_trials}_"
    ...
    train_log = args.log_dir / "native_stage1_learned_som_train_20260425.log"
    if args.force or not checkpoint.exists():
        train_run = _run_command(train_cmd, train_log, args.repo)
        if train_run["returncode"] != 0:
            raise RuntimeError(f"Stage1 learned-SOM training failed: {train_log}")
    else:
        train_run = {
            "cmd": train_cmd,
    ...
    heldout_log = args.log_dir / "native_stage1_learned_som_heldout_20260425.log"
    if args.force or not heldout_eval.exists():
        ...
        if heldout_run["returncode"] != 0:
            raise RuntimeError(f"Stage1 learned-SOM heldout eval failed: {heldout_log}")
    else:
        heldout_run = {
            "cmd": heldout_cmd,
    ...
    localizer_row = Row(
        ...
        name="sensory_only_g0_learned_som_checkpoint",
    ...
    localizer = out_dir / (
        "richter_dampening_learned_som_seed4242_reps4_rate100_sigma22_"
        "sensory_only_g0.json"
    )
    localizer_log = args.log_dir / "native_richter_learned_som_sensory_only_20260425.log"
    ...
    rows = [
        EvalRow(
            "learned_som_only_g2_r0",
            2.0,
            0.0,
            "learned",
            "primary_preregistered_learned_som_only",
        ),
        EvalRow(
            "learned_som_shifted_control_g2_r0",
            2.0,
            0.0,
            "learned-shifted",
            "wrong_hpred_mapping_control",
        ),
        EvalRow(
            "learned_som_disabled_control_g2_r0",
            2.0,
            0.0,
            "disabled",
            "som_feedback_disabled_control",
        ),
        EvalRow(
            "learned_som_plus_fixed_direct_g2_r03333333333333333",
            2.0,
            1.0 / 3.0,
            "learned",
            "analysis_only_preregistered_direct_plus_learned_som",
        ),
    ]
    ...
    for eval_row in rows:
        ...
        log_path = args.log_dir / f"native_richter_learned_som_{eval_row.name}_20260425.log"
    ...
    summary_md = (
        args.stage3_dir
        / "stage3_native_richter_learned_som_feedback_20260425.md"
    )
    summary: Dict[str, Any] = {
        "analysis_kind": "native_richter_prediction_only_learned_hpred_v1som_feedback",
        ...
        "training_constraints": {
            "ctx_pred_objective": "predict future lower-level V1_E trailer template from context/leader",
            "feedback_objective": "learn H_pred->V1_SOM future sensory template route from training schedule",
    ...
    for row in rows_out:
        print(
            ...
            f"beats_baseline_all_three={row['criteria']['beats_original_natural_baseline_all_three']}"
        )
    return 0
