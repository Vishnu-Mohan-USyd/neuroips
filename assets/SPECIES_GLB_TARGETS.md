...
targets for runtime-sized exports: mouse 800, zebrafish 1200, and 1500 for
rat, marmoset, and macaque. When `--brainglobe-root` is provided, mouse uses
BrainGlobe `allen_mouse_25um_v1.2` meshes and rat uses
`whs_sd_rat_39um_v1.2` meshes. The generator still names every GLB node from the
frontend catalog mesh keys, so changing source meshes or face targets must not
change catalog joins.
