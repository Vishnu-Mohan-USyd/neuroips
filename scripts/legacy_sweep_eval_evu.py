...
def main():
    ...
    if res.unexpected_keys:
        print(f"PORT: {len(res.unexpected_keys)} unexpected keys ignored")
    if res.missing_keys:
        print(f"PORT: {len(res.missing_keys)} missing keys default-init")
    net.eval()
    ...
    net_off = LaminarV1V2Network(model_cfg)  # PORT: current ctor takes only cfg
    net_off.load_state_dict(ckpt['model_state'], strict=False)  # PORT: strict=False for legacy ckpts
    net_off.eval()
