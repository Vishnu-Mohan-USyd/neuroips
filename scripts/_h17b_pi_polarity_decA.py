"""Phase 7 Section 2 — Dec A cross-check on pi polarity flip.

Re-runs the Phase 5 H_pi test on R1+R2 with Dec A readout alongside Dec C.

Original Phase 5 (Dec C) finding: pi-matching the HMM C1 pe≤5° subset to the
HMS-T paired-fork qualifying subset's pi-Q75 filter flips Δr_stimch from
−0.043 to +0.135. Phase 7 Section 2 verifies whether Δ_decA shows a parallel
pattern.

50 batches × bs=1024 = 51200 trials × paired-fork (intact, fb=1.0).
"""
...
KEYS = ("pe_A", "pi_isi", "is_amb_probe", "actual_ori_probe", "ori_minus1", "ori_minus2",
        "true_ch_ex", "true_ch_unex", "pred_ch",
        "r_stimch_A", "r_stimch_B",
        "decC_correct_A", "decC_correct_B",
        "decA_correct_A", "decA_correct_B")
buf = {k: [] for k in KEYS}

print(f"[forward] {N_BATCHES} batches × bs={BATCH_SIZE} = {N_BATCHES * BATCH_SIZE} trials  paired-fork", flush=True)
for bi_b in range(N_BATCHES):
    ...
    decC_A_top1 = decC(r_probe_A).argmax(-1)
    decC_B_top1 = decC(r_probe_B).argmax(-1)
    decA_A_top1 = decA(r_probe_A).argmax(-1)
    decA_B_top1 = decA(r_probe_B).argmax(-1)
    decC_corr_A = (decC_A_top1 == true_ch_ex).float()
    decC_corr_B = (decC_B_top1 == true_ch_unex).float()
    decA_corr_A = (decA_A_top1 == true_ch_ex).float()
    decA_corr_B = (decA_B_top1 == true_ch_unex).float()
    ...
    buf["decC_correct_A"].append(decC_corr_A.cpu().numpy())
    buf["decC_correct_B"].append(decC_corr_B.cpu().numpy())
    buf["decA_correct_A"].append(decA_corr_A.cpu().numpy())
    buf["decA_correct_B"].append(decA_corr_B.cpu().numpy())
    if (bi_b + 1) % 10 == 0:
...
S_C1   = keep & (data["pe_A"] <= 5.0)
S_HMST = keep & is_3march & (data["pe_A"] <= 5.0) & (data["pi_isi"] >= pi_q75)
S_C1_pi = S_C1 & (data["pi_isi"] >= pi_q75)
...
def report(name, mask):
    ...
    decC_ex = float(data["decC_correct_A"][mask].mean()); decC_un = float(data["decC_correct_B"][mask].mean())
    decA_ex = float(data["decA_correct_A"][mask].mean()); decA_un = float(data["decA_correct_B"][mask].mean())
    pi_mean = float(data["pi_isi"][mask].mean())
    return {"n": n, "pi_mean": pi_mean,
            "delta_r_stimch": rsA - rsB,
            "delta_decC": decC_ex - decC_un,
            "delta_decA": decA_ex - decA_un,
            "decC_ex": decC_ex, "decC_un": decC_un,
            "decA_ex": decA_ex, "decA_un": decA_un}


print("\n========== Phase 7 §2 — pi polarity flip on Dec A vs Dec C ==========")
print(f"  {'subset':45s}  {'n':>5s}  {'pi':>6s}  {'Δr_stimch':>10s}  "
      f"{'Δ_decC':>9s}  {'Δ_decA':>9s}")
out = {}
for label, mask in [
    ("HMM C1 pe<=5° (no pi filter)", S_C1),
    ("HMS-T paired-fork qual", S_HMST),
    ("HMM C1 pe<=5° + pi-Q75 (matched)", S_C1_pi),
]:
    r = report(label, mask)
    if r is None:
        print(f"  {label:45s}  insufficient")
        continue
    print(f"  {label:45s}  {r['n']:5d}  {r['pi_mean']:.3f}  "
          f"{r['delta_r_stimch']:+10.4f}  {r['delta_decC']:+9.4f}  {r['delta_decA']:+9.4f}")
    out[label] = r

# Δ_decA polarity flip verdict
if all(k in out for k in [
    "HMM C1 pe<=5° (no pi filter)", "HMS-T paired-fork qual", "HMM C1 pe<=5° + pi-Q75 (matched)"]):
    decA_C1 = out["HMM C1 pe<=5° (no pi filter)"]["delta_decA"]
    decA_HMST = out["HMS-T paired-fork qual"]["delta_decA"]
    decA_C1_pi = out["HMM C1 pe<=5° + pi-Q75 (matched)"]["delta_decA"]
    decC_C1 = out["HMM C1 pe<=5° (no pi filter)"]["delta_decC"]
    decC_HMST = out["HMS-T paired-fork qual"]["delta_decC"]
    decC_C1_pi = out["HMM C1 pe<=5° + pi-Q75 (matched)"]["delta_decC"]
    rs_C1 = out["HMM C1 pe<=5° (no pi filter)"]["delta_r_stimch"]
    rs_HMST = out["HMS-T paired-fork qual"]["delta_r_stimch"]
    rs_C1_pi = out["HMM C1 pe<=5° + pi-Q75 (matched)"]["delta_r_stimch"]

    print(f"\n  H_pi verdict on Δr_stimch (channel-resolved): "
          f"flips {rs_C1:+.4f} → {rs_C1_pi:+.4f} (target {rs_HMST:+.4f}). "
          f"{'CONFIRMED' if (rs_C1 < 0 and rs_C1_pi > 0) else 'NOT CLEAN FLIP'}")
    print(f"  H_pi verdict on Δ_decC: flips {decC_C1:+.4f} → {decC_C1_pi:+.4f} (HMS-T {decC_HMST:+.4f})")
    print(f"  H_pi verdict on Δ_decA: shifts {decA_C1:+.4f} → {decA_C1_pi:+.4f} (HMS-T {decA_HMST:+.4f})")

with open("/tmp/h17b_decA_pi_polarity.json", "w") as f:
    json.dump(out, f, indent=2)
print("\n[save] /tmp/h17b_decA_pi_polarity.json")
