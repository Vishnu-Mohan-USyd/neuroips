...
def load_model(
    ...
    if "config" in ckpt and "model" in ckpt["config"]:
        ...
        # PORT: strip legacy ModelConfig keys (matches load_config in src/config.py)
        for legacy_key in ('mechanism', 'n_basis', 'max_apical_gain', 'tau_vip', 'simple_feedback'):
            model_raw.pop(legacy_key, None)
        model_fields = set(ModelConfig.__dataclass_fields__.keys())
        model_cfg = ModelConfig(**{k: v for k, v in model_raw.items() if k in model_fields})
    ...
    net = LaminarV1V2Network(model_cfg)  # PORT: current ctor takes only cfg
    res = net.load_state_dict(ckpt["model_state"], strict=False)  # PORT: strict=False for legacy ckpts
    if res.unexpected_keys:
        logger.info(f"PORT: {len(res.unexpected_keys)} unexpected keys ignored")
    if res.missing_keys:
        logger.info(f"PORT: {len(res.missing_keys)} missing keys default-init")
    net.to(device)
    net.eval()
...
def feedback_disabled(net: LaminarV1V2Network):
    ...
    """
    # PORT: net.feedback (legacy EmergentFeedbackOperator) may not exist on
    # current simple_feedback path; treat absent fb as "nothing to save here".
    fb = getattr(net, 'feedback', None)
...
def analyze_one(
    ...
    # PORT: skip sanity_check_ablation when net.feedback module is absent
    # (current simple_feedback path doesn't have legacy EmergentFeedbackOperator;
    # ablation in feedback_disabled() works via hasattr guards on legacy attrs).
    if hasattr(net, 'feedback'):
        sanity = sanity_check_ablation(net, device)
    else:
        sanity = {
            "drive_on_max_abs": float('nan'),
            "drive_off_max_abs": float('nan'),
            "drive_on_sum": float('nan'),
            "drive_off_sum": float('nan'),
            "ablation_zero": True,  # ablation works via feedback_scale=0
        }
        logger.info("PORT: skipping sanity_check_ablation (no net.feedback module)")
