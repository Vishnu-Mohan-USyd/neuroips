...
def decB_acc_5fold(r: np.ndarray, true_ch: np.ndarray, n_folds: int = 5,
                   seed: int = 42) -> tuple[float | None, int, list[float]]:
    """Decoder B: 5-fold CV nearest-centroid accuracy on r vs true_ch.

    Mirrors `r1r2_paradigm_readout.cv_nearest_centroid_per_trial` exactly:
    deterministic permutation seeded by `seed`, fold_size = n // n_folds, trials
    in the trailing remainder are excluded. Returns (mean_acc, n_used, fold_accs).
    """
    n = int(r.shape[0])
    if n < n_folds:
        return None, 0, []
    ...
    fold_size = n // n_folds
    n_used = fold_size * n_folds
    R = torch.from_numpy(np.ascontiguousarray(r)).float()
    Y = torch.from_numpy(np.ascontiguousarray(true_ch)).long()
    accs: list[float] = []
    for fold in range(n_folds):
        test_idx = perm[fold * fold_size:(fold + 1) * fold_size]
        train_idx = torch.cat([perm[:fold * fold_size],
                               perm[(fold + 1) * fold_size:n_used]])
        train_X, train_y = R[train_idx], Y[train_idx]
        test_X, test_y = R[test_idx], Y[test_idx]
        classes_present = train_y.unique()
        cent_list, cent_labels = [], []
        for c in classes_present.tolist():
            mask = train_y == c
            if mask.any():
                cent_list.append(train_X[mask].mean(dim=0))
                cent_labels.append(c)
        centroids = torch.stack(cent_list, dim=0)
        cent_labels_t = torch.tensor(cent_labels, dtype=test_y.dtype)
        d2 = ((test_X.unsqueeze(1) - centroids.unsqueeze(0)) ** 2).sum(dim=-1)
        nearest = d2.argmin(dim=-1)
        preds = cent_labels_t[nearest]
        accs.append(float((preds == test_y).float().mean()))
    return float(np.mean(accs)), int(n_used), accs
...
def eval_NEW(checkpoint: str, config: str, decoder_c_path: str,
    ...
    cor_ex_R: list[np.ndarray] = []   # raw r_probe for Dec B 5-fold
    cor_ex_Y: list[np.ndarray] = []
    cor_unex_R: list[np.ndarray] = []
    cor_unex_Y: list[np.ndarray] = []
    per_N: list[dict] = []
    ...
    R_ex = np.concatenate(cor_ex_R, axis=0); Y_ex = np.concatenate(cor_ex_Y)
    R_unex = np.concatenate(cor_unex_R, axis=0); Y_unex = np.concatenate(cor_unex_Y)
    decB_ex_acc, decB_ex_n, decB_ex_folds = decB_acc_5fold(R_ex, Y_ex, seed=seed_base)
    ...
    return {
        ...
        'decB_acc_ex': decB_ex_acc, 'decB_acc_unex': decB_unex_acc,
...
def eval_M3R(checkpoint: str, config: str, decoder_c_path: str,
    ...
    out['decB_acc_ex'] = out['expected_decB_acc']
    out['decB_acc_unex'] = out['unexpected_decB_acc']
...
def eval_VCD(checkpoint: str, config: str, decoder_c_path: str,
    ...
    b_e, b_e_n, b_e_folds = decB_acc_5fold(r_e, t_e, seed=seed)
    ...
    return {
        ...
        'decB_acc_ex': b_e, 'decB_acc_unex': b_u,
        'decB_n_used_ex': b_e_n, 'decB_n_used_unex': b_u_n,
        'decB_per_fold_ex': b_e_folds, 'decB_per_fold_unex': b_u_folds,
        'decA_delta': a_e - a_u,
        'decC_delta': c_e - c_u,
        'decB_delta': decB_delta,
...
def main() -> None:
    ...
    # --- Summary table ---
    print("\n" + "=" * 92, flush=True)
    print(" Strategy            |   n_ex |  n_unex |  decA Δ  |  decB Δ  |  decC Δ  | native",
          flush=True)
    print("-" * 92, flush=True)
    for key in ['NEW', 'M3R', 'HMS', 'HMS-T', 'P3P', 'VCD']:
        ...
        print(f" {key:<19s} | {str(ne):>6} | {str(nu):>7} | {a_s:>8} | {b_s:>8} | {c_s:>8} "
