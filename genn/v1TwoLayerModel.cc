...
public:
    ...
    SET_ROW_BUILD_CODE(
        ...
        "const unsigned int preX = preSite % preSide;\n"
        "const unsigned int preY = preSite / preSide;\n"
        "const scalar preNormX = (((scalar)preX) + 0.5) / ((scalar)preSide);\n"
        "const scalar preNormY = (((scalar)preY) + 0.5) / ((scalar)preSide);\n"
        "const scalar preFieldX = sin(6.283185307179586 * preNormX) + (0.60 * cos(6.283185307179586 * preNormY)) + (0.35 * sin(6.283185307179586 * (preNormX + preNormY)));\n"
        "const scalar preFieldY = cos(6.283185307179586 * preNormX) - (0.60 * sin(6.283185307179586 * preNormY)) + (0.35 * cos(6.283185307179586 * (preNormX - preNormY)));\n"
        "scalar preOri = 0.5 * atan2(preFieldY, preFieldX);\n"
        "if(preOri < 0.0) {\n"
        ...
        "            continue;\n"
        "        }\n"
        "        const scalar postNormX = (((scalar)postX) + 0.5) / ((scalar)postSide);\n"
        "        const scalar postNormY = (((scalar)postY) + 0.5) / ((scalar)postSide);\n"
        "        const scalar postFieldX = sin(6.283185307179586 * postNormX) + (0.60 * cos(6.283185307179586 * postNormY)) + (0.35 * sin(6.283185307179586 * (postNormX + postNormY)));\n"
        "        const scalar postFieldY = cos(6.283185307179586 * postNormX) - (0.60 * sin(6.283185307179586 * postNormY)) + (0.35 * cos(6.283185307179586 * (postNormX - postNormY)));\n"
        "        scalar postOri = 0.5 * atan2(postFieldY, postFieldX);\n"
        "        if(postOri < 0.0) {\n"
        "            postOri += 3.14159265358979323846;\n"
        "        }\n"
        "        scalar delta = fabs(preOri - postOri);\n"
        "        delta = fmin(delta, 3.14159265358979323846 - delta);\n"
        "        const scalar similarity = 0.5 * (1.0 + cos(2.0 * delta));\n"
        "        scalar threshold = baseSimilarity - (distancePenalty * ((scalar)(((dx < 0) ? -dx : dx) + ((dy < 0) ? -dy : dy))));\n"
        "        if(threshold < 0.0) {\n"
        "            threshold = 0.0;\n"
        "        }\n"
...
constexpr double kDefaultL23EEStdpAminus = 0.000110;
...
constexpr double kDefaultL23SOMHomeostaticEta = 0.000020;
constexpr double kDefaultL23PVHomeostaticTargetHz = 25.0;
constexpr double kDefaultL23SOMHomeostaticTargetHz = 15.0;
    ...
    unsigned int radius)
{
    return {
        ...
        {"postNeuronsPerSite", post_neurons_per_site},
        {"radius", radius},
        {"baseSimilarity", v1_genn::kOrientationSimilarityThreshold},
        ...
        for(int dy = -static_cast<int>(v1_genn::kFeedforwardRadius); dy <= static_cast<int>(v1_genn::kFeedforwardRadius); dy++) {
            ...
            for(int dx = -static_cast<int>(v1_genn::kFeedforwardRadius); dx <= static_cast<int>(v1_genn::kFeedforwardRadius); dx++) {
                ...
                const double threshold = std::max(
                    0.0,
                    v1_genn::kOrientationSimilarityThreshold
                        - (v1_genn::kOrientationDistancePenalty * static_cast<double>(manhattan_distance)));
                if(similarity < threshold) {
                    continue;
                }
                ...
                for(unsigned int post_cell = 0; post_cell < v1_genn::kL23EPerSite; post_cell++) {
                    edges.emplace_back(
                        pre_id,
                        (post_site * v1_genn::kL23EPerSite) + post_cell);
                }
            }
        }
    }
...
void simulate(GeNN::ModelSpec &model, GeNN::Runtime::Runtime &runtime)
{
    ...
    const std::size_t total_trial_count =
        ...
        * static_cast<std::size_t>(training_epochs + 4u);
    ...
    auto runSweep = [&](const std::string &label,
        ...
        (void)label;
        runtime.setDynamicParamValue(l4e_to_l23e, "Aplus", learning ? stdp_aplus : 0.0);
        runtime.setDynamicParamValue(l4e_to_l23e, "Aminus", learning ? stdp_aminus : 0.0);
        runtime.setDynamicParamValue(
            l23e_to_l23e,
            "Aplus",
            (learning && l23ee_plasticity_enabled) ? l23ee_stdp_aplus : 0.0);
        runtime.setDynamicParamValue(
            l23e_to_l23e,
            "Aminus",
            (learning && l23ee_plasticity_enabled) ? l23ee_stdp_aminus : 0.0);
        runtime.setDynamicParamValue(
            l23pv_to_l23e,
            "TargetHz",
        ...
        runtime.setDynamicParamValue(
            l23pv_to_l23e,
            "Eta",
            (learning && l23pv_homeostatic_enabled) ? l23pv_homeostatic_eta : 0.0);
        runtime.setDynamicParamValue(
            l23som_to_l23e,
            "Eta",
            (learning && l23som_homeostatic_enabled) ? l23som_homeostatic_eta : 0.0);
        ...
        for(unsigned int orientation_index = 0; orientation_index < orientation_count; orientation_index++) {
            ...
            const unsigned int phase_slot = learning ? ((phase_cycle_offset + orientation_index) % 4u) : 0u;
    ...
    runSweep("baseline", &baseline_trials, false, 0u, -1.0);
    for(unsigned int epoch = 0; epoch < training_epochs; epoch++) {
        runSweep("training", nullptr, true, epoch, -1.0);
    }
    runSweep("post", &post_trials, false, 0u, -1.0);
    ...
    if(l23som_context_output_scale != 1.0) {
        scaleSomOutputs(l23som_context_output_scale);
    }

    runSweep("center_validation", &center_validation_trials, false, 0u, kDefaultCenterStimulusRadiusSites);
    runSweep("broad_validation", &broad_validation_trials, false, 0u, -1.0);
